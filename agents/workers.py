...
def _get_rag_context(state: dict, keywords: list[str]) -> str:
    """从 RAG 索引中检索相关上下文 + 历史负样本避坑提示。"""
    parts = []
    # RAG 代码上下文
    ...
    if chunks:
        try:
            ...
            results = search(" ".join(keywords), chunk_objs, top_k=3, load_parents=True)
            if results:
                parts.append("## 相关代码上下文（RAG 检索）\n" + "\n".join(r.to_context() for r in results))
        except Exception:
            pass
    # 历史负样本避坑提示
    try:
        from feedback.retriever import build_negative_context
        cat = keywords[0] if keywords else ""
        neg_ctx = build_negative_context(cat, keywords)
        if neg_ctx:
...
def security_worker(state: ReviewState) -> ReviewState:
    ...
    try:
        rag_ctx = _get_rag_context(state, ["api", "auth", "token", "password", "session", "sql", "exec", "eval"])
...
def performance_worker(state: ReviewState) -> ReviewState:
    ...
    client = get_llm_client()
    try:
        rag_ctx = _get_rag_context(state, ["loop", "query", "cache", "memory", "n+1", "algorithm", "complexity"])
        raw = client.chat_with_lint_context(
            code=state["code"],
            lint_results="",
...
def business_logic_worker(state: ReviewState) -> ReviewState:
    ...
    try:
        ...
        rag_ctx = _get_rag_context(state, ["class", "interface", "error", "exception", "log", "return", "validation"])
...
def architecture_worker(state: ReviewState) -> ReviewState:
    ...
    try:
        metrics = state.get("code_metrics", {})
        metrics_text = metrics_to_text(metrics) if metrics else "无法获取代码度量数据"
        rag_ctx = _get_rag_context(state, ["import", "module", "dependency", "interface", "layer", "abstract"])
        user_content = f"""## 待审查代码
```python
{state["code"]}
```
